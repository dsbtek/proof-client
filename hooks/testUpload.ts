...
import { base64ToBlob, blobToBase64, blobToBuffer, dateTimeInstance, getConnectionType } from '@/utils/utils';
...
function useTestupload() {
    ...
    const { storage, lookAway, handsOut, filename, testSteps, testStepsFiltered, timerObjs, testingKit, startTime, endTime, signature, barcode, trackingNumber, shippingLabel, barcodeKit, detectKit, proofId, faceCompare, faceScans, imageCaptures, passport, governmentID, idDetails } = useSelector(testData);
    const { participant_id } = useSelector(authToken);
    const { first_name, last_name } = useSelector(appData);
    const { osName, osVersion, deviceModel, deviceType, deviceVendor } = useGetDeviceInfo()
    ...
    const testUpload = useCallback(async () => {
        try {
            ...
            const response = await fetch(`${process.env.NEXT_PUBLIC_BEAM_SERVICE_URL}/send-results`, {
                method: "POST",
                headers: {
                    "Accept": "*/*",
