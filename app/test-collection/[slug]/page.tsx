...
import { detectBarcodes, uploadVideoToS3, createPresignedUrl } from './action';
import { base64ToFile, blobToBase64, blobToBuffer, dateTimeInstance, fileToBase64 } from '@/utils/utils';
...
function Test() {
  ...
  const barcodeCapture = useCallback(async () => {
    try {
      ...
      const readers = ['ean_13_reader', 'qr_code_reader', 'code_128_reader', 'code_39_reader', 'upc_a_reader', 'upc_e_reader', 'ean_8_reader', 'pdf417_reader', 'aztec_reader', 'data_matrix_reader', 'itf_reader', 'code_93_reader'];
      ...
      // html5QrCode.scanFileV2(imageFile!, false).then((response) => {
      //   console.log('response', response);
      //   if (response.decodedText.length > 0) {
      //     setBarcode(response.decodedText);
      //     setBarcodeUploaded(true);
      //     dispatch(saveBarcode(response.decodedText));
      //   } else {
      ...
      Quagga.decodeSingle({
        ...
        src: imageSrc!
        // src: `/images/Proof-SS/16.png`
      }, function (result: any) {
        console.log("result-->", result);
        if (result !== undefined && result.codeResult !== undefined) {
  ...
  }, [TEST_UPLOAD, TEST_VID_UPLOAD, activeStep, cameraRef, capturedVideo, dispatch, endTest, endTime, handleDialog, participant_id, pathname, record, router, showTimer, startTime, status, test, testStart, testSteps, testStepsFiltered, testingKit, testingKit.kit_id, time, timerObjs, timerStep])
    ...
    <>
      ...
      <div className="test-container">
        ...
        <div className='test-content'>
          <BarcodeCaptureModal show={showBCModal} barcode={barcode} barcode2={barcode2} barcodeImage={barcodeImage} barcodeUploaded={barcodeUploaded} step={activeStep} totalSteps={test.length} recapture={() => setShowBCModal(false)} closeModal={closeBCModal} />
          <Webcam
