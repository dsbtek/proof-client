...
function Test() {
  ...
  const endTest = useCallback(async () => {
    ...
    console.log(isFinal, "isFinal should be true");
    setMuted(true);
  ...
  const handleNextStep = useCallback(() => {
    console.log(
      mediaRecorderRef,
      "log of what mediaRecorderRef is in handleNextStep"
    );
    if (activeStep === test.length) {
      ...
      if (
        ...
        console.log("we entered this point in handleNextstep1");
        const newActiveStep = test[activeStep].step;
        setActiveStep(newActiveStep);
      } else if (time === 0 && timerStep !== activeStep) {
        console.log("we entered this point in handleNextstep2");
  ...
  useEffect(() => {
    const handleStreaming = async () => {
      console.log("started streaming again");
      ...
      if (
        ...
        if (mediaRecorderRef.current) {
          console.log(mediaRecorderRef.current, "there is mediaRecorderRef");
          mediaRecorderRef.current.stop();
          ...
          console.log(
            ...
            " after stopping and removing mediaRecorderRef"
          );
        }
    ...
    const handleDataAvailable = async ({ data }: BlobEvent) => {
      try {
        if (data.size > 0) {
          console.log("step ", activeStep, " stream data--->", data);
          console.log(isFinal.current, "is final result");
          // Stop the MediaRecorder and starts a new one, to maintain the stream data integrity
          if (isFinal.current === false) {
            console.log("we got in here... lets see");
            handleStreaming();
          }
    ...
    if (status === "previewing") {
      console.log("started media recorder again");
      if (isFinal.current === false) {
        console.log(" we shouldn't be here .... started media recorder again");
