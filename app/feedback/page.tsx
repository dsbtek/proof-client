...
const FeedbackRating = () => {
    const data = useSelector(
        (state: { preTest: { preTestFeedback: Section[] } }) =>
    ...
    const handleNextClick = async () => {
        const selectedOption =
            currentQuestion?.question_type === 'Pick list'
                ? radioOptions.find((option) => option.id === selectedRadio)
                      ?.label
                : textResponse;
    ...
    return (
        <AppContainer
            header={<AppHeader title={data[0].sectionName} hasMute={false} />}
            body={
                <div
                    className="container-test-collection"
