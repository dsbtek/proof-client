...
import { proofPassData, proofPassFilter } from '../../utils/appData'
import { SingleValue } from "react-select";
import { setReDirectToProofPass, setHistoryData, historyData, appData } from "@/redux/slices/appConfig";
...
const ProofPass = () => {
  ...
  const filteredData = selectedFilter
    ? history?.dtests?.filter((item: any) => item?.ServiceType === selectedFilter.value)
    : history?.dtests;
  ...
  return (
    <div className="container">
      ...
      <p style={{ marginTop: '16px', marginBottom: '16px' }}>
        ADD New PROOFpass: Click the `Upload` button above to add a new PROOFpass.
      </p>

      <Link className="links" href={photo ? "/identity-profile/sample-facial-capture" : "/identity-profile/id-detection/step-1"}>
        <Button blue onClick={updateRedirection}>{"UPLOAD"}</Button>
      </Link>
      <p style={{ marginTop: '16px' }}>
        VIEW a PROOFpass: Click the `Filters` button below and select the type of PROOFpass you want to find in your list
      </p>
      {
        isLoading ?
          <Loader_ />
            ...
            <div className="proof-pass-list-container">
              {filteredData?.length > 0 ? (
                filteredData.map((item: any, index: number) => (
                  <div key={index} className={`proof-pass-list ${item.DrugTestResultStatus ? item.DrugTestResultStatus : "Inconclusive"}`}>
                    <div className="wrap-proof-pass-item">
                      <p>{item.submitteddate}</p>
                      <p>{item.ServiceType}</p>
                      <p>{item.DrugTestResultStatus ? item.DrugTestResultStatus : "Not Applicable"}</p>
                    </div>
                    <Link href={`/proof-pass/${index}`}><AiOutlineRight /></Link>
                  </div>
                ))
              ) : (
