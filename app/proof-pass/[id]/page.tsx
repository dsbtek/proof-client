    ...
    return (
        <div className="container">
            <div className="proof-pass-header">
                <AppHeader title="" hasMute={false} />
            </div>
            <div
                className={`proofpass-detail ${
                    proofpass?.DrugTestResultStatus
                        ? proofpass?.DrugTestResultStatus
                        : 'Inconclusive'
                } proofpass-text`}
            >
                <div className="proof-pass-head">
                    <h1>
                        {proofpass?.DrugTestResultStatus?.toLocaleUpperCase()}
                    </h1>
                    <p>{proofpass?.TestPanel}</p>
                    <p>{`This service took place ${getNumberOfDays(
                        proofpass?.servicedate,
                    )} days ago`}</p>
                </div>
                <div
                    className="wrap-result-item"
                    style={{
                        display: 'flex',
                ...
                >
                    <p>Identity:</p>{' '}
                    <p>{proofpass?.GovernmentPhotoID || 'Not Applicable'}</p>
                </div>
                {proofpass?.DrugTestResultStatus === 'Negative' ? (
                    <div className="proof-pass-item-border border-green"></div>
                ) : (
                ...
                >
                    <p>Collection:</p>{' '}
                    <p>
                        {proofpass?.CollectionDesignation || 'Not Applicable'}
                    </p>
                </div>
                {proofpass?.Images && (
                    <div>
