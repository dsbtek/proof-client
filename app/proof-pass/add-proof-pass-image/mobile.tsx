...
const ScanLabReport = () => {
  ...
  useEffect(() => {
    const initializeFormValues = async () => {
      ...
      if (participant_id && pin) {
        ...
        const decryptedPin = decryptData(
          pin,
          process.env.NEXT_PUBLIC_SECRET_KEY
        );
  ...
  }, [barcodeKit, compressImage, facialPercentageScore, facialUrl, idUrl, imageNames, proofPassData?.collectionDate, proofPassData?.firstName, proofPassData?.lastName, proofPassData?.panel, proofPassData?.result?.value, proofPassData?.typeOfService?.value, scanReports, trackingNumber, userData.Address_Line_1, userData.City, userData.State, userData.Zip_Code, userData.birth_date, userData.photo, userData.proof_id_value]);
      ...
      {showModal && (
        <DocumentTypeModal
          ...
          triggerCameraFileUpload={() => {
            ...
            if (fileUpload_) {
              setFileUpload(false);
              dispatch(setScanReport({ [imageName]: capturedImage }));
              return
            } else {
              setShowCamera(true);
            }
      ...
      <div className="proof-pass-camera-wrapper">
        ...
        {showCamera && !isLoading && (
          <Button
            ...
            style={{ position: "absolute", bottom: "2rem", width: "90%" }}
          >
            Capture
          </Button>
        )}
        {scanReports.length > 0 || showCamera ? (
          ...
          >
            Press the Add button with + to take a photo of your test result.
          </p>
        )}

        {!showCamera && !isLoading && <ThumbnailGallery images={scanReports} />}
