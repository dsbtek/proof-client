...
const DatePicker = ({ title, date, onDateSelect }: Callendar) => {
    ...
    const [selectedMonth, setSelectedMonth] = useState<number>(new Date().getMonth()); // 0-based month index
    ...
    useEffect(() => {
        const formattedDate = `${formatDay(selectedDay)}-${formatMonth(selectedMonth + 1)}-${selectedYear}`;
    ...
    const handleClickOutside = (event: MouseEvent) => {
        if (datePickerRef.current && !datePickerRef.current.contains(event.target as Node) && buttonRef.current && !buttonRef.current.contains(event.target as Node)) {
    ...
    return (
        <div className='date-picker-container'>
            <DatePickerBtn ref={buttonRef} onClick={toggleCallendarView} classname="select-w">
                <span>{title}: {selectedDate}</span>
                {isDatePickerVisible ? <AiOutlineUp size={20} /> : <AiOutlineDown size={20} />}
            </DatePickerBtn>
            <br />
            {isDatePickerVisible && (
