...
const ThumbnailGallery = ({ images }: ThumbnailGalleryProps) => {
  ...
  const extractImages = images?.flatMap((obj: { [s: string]: string; } | ArrayLike<string>) => Object.values(obj));
      ...
      {extractImages.map((report, index) => (
        <div key={index} className="thumbnail">
          ...
          <div className="thumbnail-overlay">
            <Button classname='thumbnail-overlay-btn-view' onClick={() => handlePreview(index)}>View</Button>
            <Button classname='thumbnail-overlay-btn-remove' onClick={() => handleRemove(index)}>Remove</Button>
          </div>
        </div>
      ))}
      {isModalOpen && (
